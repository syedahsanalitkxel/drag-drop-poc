...
class AddAssessment extends Component<any, State> {
	constructor(props: any) {
		...
		this.state = {
			...
			fathSelected: '',
			itemsElements: [{ statement: '', behaviur: '', scaling: '' },
			{ statement: '', behaviur: '', scaling: '' },
			{ statement: '', behaviur: '', scaling: '' },
			{ statement: '', behaviur: '', scaling: '' }, { statement: '', behaviur: '', scaling: '' }],
			lists: {},
	...
	public entityhandleChange(event: any) {
		...
		entities.forEach((entity: any) => {
			if (entity.value === event.target.value) {
				if (entity.isChecked === true) { this.setState({ entitySelect: this.state.entitySelect - 1 }) }
				else {
	...
	public render() {
		...
		const addAssessmentSchema = Yup.object().shape({
			...
			expstatement: Yup.array().of(Yup.object().shape({
				expstatement: Yup.string()
					.min(1, "Too Short!")
					.max(250, "Too Long!")
					.required("Required"),
		...
		const renderForm = (formikprops: any) => (<form onSubmit={formikprops.handleSubmit} className={'form'}>
			<div className="row">
					...
					<div className="wrapper wrapper-content animated fadeInRight">
						<div className="ibox">
							<div className="ibox-content">
								<div className="form-group  row"><label className="col-sm-2 col-form-label font-bold" >Defination</label>
									<div className="col-sm-10">
										...
										<Input
											...
											tag={Field}
											invalid={!!(formikprops.touched.definiation && formikprops.errors.definiation)}
										/>
								...
								<div className="form-group row"><label className="col-sm-2 col-form-label font-bold">Category</label>
									<div className="col-sm-10">
								...
								<div className="hr-line-dashed" />
								{this.state.typeSelected === 'comtval' ? <Fragment><div className="form-group row">	<label className="col-sm-2 col-form-label font-bold">Competency</label><div className="col-sm-10">	<select className="form-control m-b col-sm-4" name="account" value={this.state.competency} onChange={this.competencyhandleChange}>	<option>Add Competency</option><option>option 2</option><option>option 3</option><option>option 4</option></select></div></div>	<div className="hr-line-dashed" /></Fragment> : null}
								<div className="form-group row">
								...
								<div className="form-group row">
									...
									<div className="col-sm-10">
										...
										{this.state.recommendAppCorporate ? null : this.state.recommendAppHigher ? null : <div className="isa_error">
											<span className="error text-danger">
								...
								<div className="form-group row">
									<label
										className="col-sm-2 col-form-label font-bold">Assesment Type</label>
								...
								{this.state.assessmentType === 'rubricval' ?
									<Fragment>
										<Assessmentelement formikprops={formikprops} tag={Field} key={0} comNumber={0} onChange={this.onelementhandleChange} />
										{
											this.state.countAssetelement > 0 ? <Fragment> {function (count, handle) {
												let arr = []
													...
													arr.push(
														<Fragment>
															<div className="ibox showMore">
																...
																<Assessmentelement formikprops={formikprops} comNumber={i + 1} tag={Field} key={i + 1} onChange={handle} />
															</div>
														</Fragment>)
												} return arr
											}(this.state.countAssetelement, this.onelementhandleChange)}</Fragment> : null
										}
									</Fragment> : null}
							</div>
						</div>
						{this.state.typeSelected === 'influval' ?
							<div className="">
								<button type="button" id="helper" onClick={this.onClickassetelement} data-toggle="tooltip" className="btn btn-outline btn-primary showmore">Add More Elements</button>
							</div> : null
						}
						<div className="m-t-15 m-b-15 button-wrapper">
							<button type="button" style={styles.btn} className="btn btn-default btn-lg" >Cancel</button>
							<button type="submit" style={styles.btn} id={'submit'} name='submit' className="btn btn-primary btn-lg">Save</button>
							<button type="button" style={styles.btn} className="btn btn-primary btn-lg">Save As Draft</button>
							<button type="button" style={styles.btn} className="btn btn-primary btn-lg">Save  Add More</button>
						</div>
					</div>
				</div>
			</div></form>
		...
		return (
			<DashboardTemplate>
				<Formik
					initialValues={{ categorySelected: this.state.categorySelected, definiation: '', expstatement: this.state.expstatement, expBehaviour: '', expscaling: '', excstatement: '', excbehaviour: '', excscaling: '', comstatement: '', combehavior: '', comscaling: '', marstatement: '', marbehaviour: '', marscaling: '', unstatement: '', unbehaviour: '', unscaling: '' }}
